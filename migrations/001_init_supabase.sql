...
-- Initial schema for the RAG pipeline — UP-ONLY version for Supabase.
--
-- IMPORTANT: This is the "Up" half of the original goose migration
-- (001_init.sql), extracted on purpose.
--
-- Why: Supabase's SQL Editor / dashboard does NOT understand goose's
-- "-- +goose Up" / "-- +goose Down" annotations — those are only
-- recognized by the `goose` CLI itself. If you paste the full
-- Up+Down file straight into Supabase, Postgres just runs it top to
-- bottom as plain SQL, which means it executes the Up section AND
-- immediately follows with the Down section (all the DROP TABLE /
-- DROP POLICY statements). Net effect: everything gets created then
-- immediately deleted in the same run, which is exactly why a later
-- query said `relation "tenants" does not exist`.
--
-- Use this file for Supabase (SQL Editor, or `supabase db push` /
-- Supabase CLI migrations). Keep using the original 001_init.sql
-- with the real `goose` CLI for local Docker, where goose correctly
-- reads only the Up section.
